...
class Timer(QObject):
    ...
    def __init__(self, parent=None, timerEdit=None):
        if timerEdit:
            self.timerEdit = timerEdit
        QObject.__init__(self, parent=parent)
        self.continue_run = True
    
    def countdown(self):
        print("starting countdown...")
        if not self.timerEdit:
            print("no timerEdit... please add it.")
        t = self.strToSec(self.timerEdit.text())
        while t != 0 and self.continue_run:
            mins, secs = divmod(t, 60)
            timeformat = '{:02d}:{:02d}'.format(mins, secs)
            self.timerEdit.setText(timeformat)
            QThread.sleep(1)
            t -= 1
        self.continue_run = True
        self.finished.emit()
...
class Window(QtWidgets.QMainWindow, Ui_TimerWindow):
    ...
    def __init__(self):
        ...
        # Thread:
        self.thread = QThread()
        self.timer = Timer(timerEdit = self.timerEdit)
        ...
        # Stop Button action:
        self.stopButton.clicked.connect(self.stop_signal.emit)
